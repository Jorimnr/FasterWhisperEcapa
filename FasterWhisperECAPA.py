...
file_vad_segments = {}  # Store all VAD segments per file for concatenation
...
for audio_file, channel_idx in zip(audio_files, range(C)):
    ...
    for seg in segments:
        ...
        # Store segment audio for concatenation
...
# Export concatenated VAD segments per file
output_dir = f"segments_{timestamp}"
os.makedirs(output_dir, exist_ok=True)

print("\nExporting concatenated VAD segments per file...")
for audio_file, channel_idx in zip(audio_files, range(C)):
    name = os.path.splitext(os.path.basename(audio_file))[0]
    
    if name in file_vad_segments and len(file_vad_segments[name]) > 0:
        # Concatenate all segments for this file
        concatenated_audio = np.concatenate(file_vad_segments[name])
        
        # Determine main speaker for filename
        main_speaker = file_main_speakers.get(name, name)
        
        output_filename = f"{output_dir}/{main_speaker}_all_vad_segments.wav"
        sf.write(output_filename, concatenated_audio, 16000)
        
        duration_sec = len(concatenated_audio) / 16000
        print(f"  Exported {main_speaker}: {len(file_vad_segments[name])} segments, {duration_sec:.1f}s total")
    else:
        print(f"  No segments found for {name}")
...
for group in similar_groups:
    ...
    # Find the best transcript in this group using multiple criteria
    best_idx = group[0]
    ...
    for idx in group[1:]:
        ...
        # 1. Always prefer non-zero score over zero score
        # 2. If both have scores, prefer significantly higher score (>0.1 difference)
        # 3. If scores similar, prefer longer text
        # 4. If text similar length, prefer earlier timestamp (likely more complete)
        
        if best_row['score'] == 0.0 and row['score'] > 0.0:
            # Current is better (has confidence)
            best_idx = idx
            best_row = row
        elif row['score'] > 0.0 and best_row['score'] > 0.0:
            # Both have confidence scores
            if row['score'] > best_row['score'] + 0.1:
                # Significantly better score
                best_idx = idx
                best_row = row
            elif abs(row['score'] - best_row['score']) < 0.1:
                # Similar scores, prefer longer or more complete text
                if len(row['text']) > len(best_row['text']) * 1.1:  # At least 10% longer
                    best_idx = idx
                    best_row = row
        # If best has score and current is 0.0, keep best
...
# Step 5: Remove cross-talk duplicates (when file_channel != assigned_speaker with overlapping time)
def remove_crosstalk_duplicates(df):
    """
    Remove segments where a speaker was captured by another person's mic,
    when that same speaker's own mic already has a better recording.
    
    Example: SPK3's mic captures SPK1 speaking, but SPK1's mic has it too -> remove SPK3's version
    """
    ...
    for i in range(len(df)):
        ...
        # Only check segments where the file channel doesn't match the assigned speaker
        # (i.e., cross-talk: SPK3 mic picking up SPK1)
        if row['file_channel'] != row['assigned_speaker']:
            assigned_spk = row['assigned_speaker']
            
            # Look for segments from the actual speaker's own mic in the same time window
            for j in range(len(df)):
                ...
                # Check if this is the same speaker on their own mic
                if other['file_channel'] == assigned_spk and other['assigned_speaker'] == assigned_spk:
                    # Check for time overlap
                    time_ovl = time_overlap(row['start_time'], row['end_time'],
                                          other['start_time'], other['end_time'])
                    
                    # Check text similarity
                    text_sim = similarity_score(row['text'], other['text'])
...
clean_df = remove_crosstalk_duplicates(clean_df)
print(f"After removing cross-talk duplicates: {len(clean_df)}")
...
# Step 7: Combine consecutive segments from the same speaker
def combine_consecutive_speaker_segments(df):
    """
    Combine consecutive rows where the same speaker continues speaking.
    Merges text and adjusts start/end times accordingly.
    """
    if len(df) == 0:
        return df
    ...
    for i in range(1, len(df)):
        ...
        # Check if same speaker in consecutive position
        if (row['assigned_speaker'] == current_row['assigned_speaker'] and 
            row['file_channel'] == current_row['file_channel']):
            # Combine: extend end time and append text
            current_row['end_time'] = row['end_time']
            current_row['text'] = current_row['text'].strip() + ' ' + row['text'].strip()
            # Keep the higher score
            current_row['score'] = max(current_row['score'], row['score'])
        else:
            # Different speaker or file, save current and start new
...
print(f"Before combining consecutive segments: {len(final_df)}")
final_df = combine_consecutive_speaker_segments(final_df)
print(f"After combining consecutive segments: {len(final_df)}")
